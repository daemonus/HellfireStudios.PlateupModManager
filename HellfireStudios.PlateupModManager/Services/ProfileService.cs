...
public class ProfileService
{
    ...
    public ModProfile CreateProfileFromMods(string name, string description, IEnumerable<InstalledMod> enabledMods)
    {
        var profile = new ModProfile
        {
            Name = name,
            Description = description,
            Mods = enabledMods.Select(m => new ProfileMod
            {
    ...
    public async Task EnsureDefaultProfilesAsync(string workshopContentPath, ModManagerService modManager)
    {
        ...
        // Clean profile — empty mod list, applying it disables everything
        if (await GetProfileAsync(CleanProfileId) == null)
        {
            var clean = new ModProfile
            {
                Id = CleanProfileId,
                Name = "Clean (No Mods)",
                Description = "Disables all mods for a vanilla PlateUp! experience.",
                Mods = []
            };
